...
class Texter_API_Endpoint_Gallery
{
    ...
    public function register_routes($namespace)
    {
        ...
        // Upload image to gallery (supports multipart form data or JSON with base64)
        register_rest_route($namespace, '/gallery/upload', [
            ...
            'args' => [
                'image' => [
                    'required' => false, // Optional because multipart uploads use $_FILES
                    'type' => 'string',
                    'description' => 'Base64-encoded image data (for JSON uploads)',
                ],
                'title' => [
                    'required' => false,
                    'type' => 'string',
     ...
     * Accepts either multipart form data or JSON with base64 image
     *
     * @param WP_REST_Request $request
     * @return WP_REST_Response|WP_Error
     */
    public function upload_image($request)
    {
        ...
        if (!empty($files['image']) && $files['image']['error'] === UPLOAD_ERR_OK) {
            ...
            // Fallback to base64 JSON (legacy support)
